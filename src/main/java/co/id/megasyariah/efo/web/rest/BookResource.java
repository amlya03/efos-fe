...
/**
 * REST controller for managing {@link co.id.megasyariah.efo.domain.Book}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class BookResource {
    ...
    public ResponseEntity<Book> partialUpdateBook(@PathVariable(value = "id", required = false) final Long id, @RequestBody Book book)
        ...
        Optional<Book> result = bookRepository
            .findById(book.getId())
            .map(existingBook -> {
                if (book.getContoh() != null) {
                    existingBook.setContoh(book.getContoh());
                }
                if (book.getNama() != null) {
                    existingBook.setNama(book.getNama());
                }
                if (book.getKelas() != null) {
                    existingBook.setKelas(book.getKelas());
                }
