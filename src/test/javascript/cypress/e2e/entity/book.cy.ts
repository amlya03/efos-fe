...
describe('Book e2e test', () => {
  ...
  describe('new Book page', () => {
    ...
    it('should create an instance of Book', () => {
      cy.get(`[data-cy="contoh"]`).type('Union gold Fresh').should('have.value', 'Union gold Fresh');

      cy.get(`[data-cy="nama"]`).type('Electronics extend').should('have.value', 'Electronics extend');

      cy.get(`[data-cy="kelas"]`).type('Sausages Credit firewall').should('have.value', 'Sausages Credit firewall');
